      ...
      <div className="w-1/2">
        ...
        <form onSubmit={handleSubmit} className="w-full mx-auto p-4 border text-lg bg-light-four/50 rounded-md shadow-xl">
          <label className="flex flex-col  md:flex-row justify-between items-center my-1">
            Email: <input className="w-full md:w-3/4 text-sm p-2 my-1 dark:text-dark-one" type="email" name="email" required
              placeholder="Enter your email" />
          </label>
          <label className="flex flex-col  md:flex-row justify-between items-center my-1">
            Password: <input className="w-full md:w-3/4 text-sm p-2 my-1 dark:text-dark-one" type="password" name="password"
              placeholder="Enter your password" />
          </label>
          <div className="flex items-end">
            <button type="submit" className="shadow my-2 block mx-auto py-2 px-4 md:px-10 hover:scale-110  font-bold hover:text-light-three rounded-md transition-all bg-light-three hover:bg-light-five text-light-five">Login</button>
            <GoogleLogin />
          </div>
